...
void check_uint32(testresult_t* result, const char* fail_msg, uint32_t actual, uint32_t expected)
{
  if(actual != expected) {
    result->errors += 1;
    printf("%s: Actual %X, expected %X\n\r", fail_msg, (unsigned int) actual, (unsigned int) expected);
  }
}
...
void perf_print_all(void) {
  ...
  printf("Perf #RVC:     %d\n\r", cpu_perf_get(10));
#else
  printf("Perf CYCLES: %d\n\r",      cpu_perf_get(SPR_PCER_CYCLES));
  printf("Perf INSTR: %d\n\r",       cpu_perf_get(SPR_PCER_INSTR));
  printf("Perf LD_STALL: %d\n\r",    cpu_perf_get(SPR_PCER_LD_STALL));
  printf("Perf JMP_STALL: %d\n\r",   cpu_perf_get(SPR_PCER_JMP_STALL));
  printf("Perf IMISS: %d\n\r",       cpu_perf_get(SPR_PCER_IMISS));
  printf("Perf WBRANCH: %d\n\r",     cpu_perf_get(SPR_PCER_WBRANCH));
  printf("Perf WBRANCH_CYC: %d\n\r", cpu_perf_get(SPR_PCER_WBRANCH_CYC));
  printf("Perf LD: %d\n\r",          cpu_perf_get(SPR_PCER_LD));
  printf("Perf ST: %d\n\r",          cpu_perf_get(SPR_PCER_ST));
  printf("Perf JUMP: %d\n\r",        cpu_perf_get(SPR_PCER_JUMP));
  printf("Perf BRANCH: %d\n\r",      cpu_perf_get(SPR_PCER_BRANCH));
  printf("Perf DELAY NOP: %d\n\r",   cpu_perf_get(SPR_PCER_DELAY_NOP));
#endif
}
