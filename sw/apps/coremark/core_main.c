...
#error "Please define a way to initialize a memory block."
	...
	if (results[0].iterations==0) { 
		...
		while (secs_passed < (secs_ret)1) {
			...
			iterate(&results[0]);
			stop_time();
			secs_passed=time_in_secs(get_time_pulp());
		}
	...
	//ee_printf("Iterations       : %lu\n\r", (long unsigned) default_num_contexts*results[0].iterations);
	ee_printf("Iterations       : %u\n\r", (unsigned int) default_num_contexts*results[0].iterations);
